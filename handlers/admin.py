...
def create_callback(callback_or_message, data, bot=None):
    """إنشاء callback جديد"""
    if hasattr(callback_or_message, 'from_user'):
        ...
        return types.CallbackQuery(
            ...
            chat_instance=str(callback_or_message.message.chat.id),
        ...
        return types.CallbackQuery(
            id=str(uuid.uuid4()),
            from_user=callback_or_message.from_user,
            chat_instance=str(callback_or_message.chat.id),
            message=types.Message(
                message_id=callback_or_message.message_id + 1,
                date=callback_or_message.date,
                chat=callback_or_message.chat,
                from_user=callback_or_message.from_user
            ),
            data=data,
            bot=bot
...
async def admin_panel(message: types.Message, db_pool):
    ...
    kb = [
        ...
        [types.InlineKeyboardButton(text="🔄 تفعيل/إيقاف التطبيقات", callback_data="manage_apps_status"),
         types.InlineKeyboardButton(text="🎮 إدارة خيارات المنتجات", callback_data="manage_options")],
...
async def toggle_app_from_options(callback: types.CallbackQuery, db_pool):
    ...
    await show_product_options(
        create_callback(callback, f"prod_options_{app_id}"),
        db_pool
    )

# ============= تبديل حالة الخيار (تفعيل/تعطيل) =============
...
async def toggle_option_status(callback: types.CallbackQuery, db_pool):
    """تفعيل أو تعطيل خيار معين"""
    option_id = int(callback.data.split("_")[2])
    
    async with db_pool.acquire() as conn:
        # جلب الحالة الحالية
        option = await conn.fetchrow(
            "SELECT product_id, name, is_active FROM product_options WHERE id = $1",
            option_id
        )
        ...
        await callback.answer(f"✅ تم {status_text} الخيار {option['name']}")
    
    # العودة لقائمة الخيارات
    await show_product_options(
        create_callback(callback, f"prod_options_{option['product_id']}"),
        db_pool
    )
...
async def edit_field_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        field_names = {'name': 'الاسم', 'quantity': 'الكمية', 'price': 'السعر', 'desc': 'الوصف'}
...
async def add_option_step_supplier_price(message: types.Message, state: FSMContext):
    """استلام السعر"""
    if not is_admin(message.from_user.id):
        return
    ...
    await state.update_data(supplier_price=price)
...
async def add_option_step_description(message: types.Message, state: FSMContext, db_pool):
    ...
    if description:
        await message.answer(f"📝 **الوصف:**\n{description}")
    
    await state.clear()
    
    # العودة لقائمة الخيارات
    await show_product_options(
        create_callback(message, f"prod_options_{product_id}", message.bot),
        db_pool
    )
...
async def delete_option_execute(callback: types.CallbackQuery, db_pool):
    ...
    await callback.answer("✅ تم تعطيل الخيار بنجاح")
    
    # العودة لقائمة الخيارات
    await show_product_options(
        create_callback(callback, f"prod_options_{product_id}"),
        db_pool
    )

# ============= باقي الكود (إدارة النقاط، المشرفين، الإحصائيات، إلخ) يبقى كما هو =============
# ... (باقي دوال إدارة النقاط، المشرفين، الإحصائيات، طلبات الشحن، طلبات التطبيقات، رفع مستوى VIP، إلخ)
# ============= إدارة النقاط =============
